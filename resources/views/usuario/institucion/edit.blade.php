            ...
            <div class="col-md-8"> <!-- Se aumenta el tamaño del formulario -->
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Formulario de Edición de Institución</h3>
                    </div>
                    <form action="{{ route('institucion.update', $institucion->id_institucion) }}" method="post" class="form">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
    ...
    <select class="form-control" name="id_municipio" id="id_municipio" required>
        ...
        @foreach($municipios as $municipio)
            <option value="{{ $municipio->id_municipio }}" 
                {{ old('id_municipio', $institucion->id_municipio) == $municipio->id_municipio ? 'selected' : '' }}>
                {{ $municipio->nombre }}
            </option>
        @endforeach
    </select>
...
<div class="form-group">
    ...
    <input type="text" class="form-control" name="zona" id="zona" placeholder="Ingresa la zona" 
           value="{{ old('zona', $institucion->zona) }}">
    @if ($errors->has('zona'))
    ...
    @endif
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="codigo" id="codigo" value="{{ $institucion->codigo }}" required>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="localizacion" id="localizacion" value="{{ $institucion->localizacion }}" required readonly>
...
<script>
    ...
    function initMap() {
        var savedLocation = {lat: parseFloat("{{ explode(',', $institucion->localizacion)[0] }}"), lng: parseFloat("{{ explode(',', $institucion->localizacion)[1] }}")};
        ...
        // Marcador inicial basado en la localización guardada
    ...
    $('#mapModal').on('shown.bs.modal', function () {
        ...
        modalMap.setCenter(map.getCenter()); // Centrar el modalMap basado en el centro del mapa principal
    });
};
